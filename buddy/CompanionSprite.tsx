...
import { friendObserver } from './observer.js';
...
export function CompanionSprite(): React.ReactNode {
  const reaction = useAppState((s: AppState) => s.companionReaction) as string | undefined;
  const mood = (useAppState((s: AppState) => s.companionMood) as 'idle' | 'excited' | 'sleepy' | 'sad' | undefined) || 'idle';
  const petAt = useAppState((s: AppState) => s.companionPetAt) as number | undefined;
  const focused = useAppState((s: AppState) => s.footerSelection === 'companion') as boolean;
  ...
  useEffect(() => {
    friendObserver.start(setAppState);
  ...
  // Mood influence on tick speed
  const effectiveTick = mood === 'sleepy' ? Math.floor(tick / 2) : mood === 'excited' ? tick * 2 : tick;

  if (reaction || petting || mood === 'excited') {
    // Excited: cycle all fidget frames fast
    spriteFrame = effectiveTick % frameCount;
  } else {
    const step = IDLE_SEQUENCE[effectiveTick % IDLE_SEQUENCE.length]!;
    if (step === -1) {
      spriteFrame = 0;
      blink = true;
    } else {
  ...
  // Breathing animation for idling
  let breathingPadding = 0;
  if (!reaction && !petting) {
    if (mood === 'idle' && (tick % 8) < 4) breathingPadding = 1;
    else if (mood === 'sleepy' && (tick % 16) < 8) breathingPadding = 1;
  }

  const body = renderSprite(companion, spriteFrame).map((line: string) => 
    (blink || mood === 'sleepy') ? line.replaceAll(companion.eye, '-') : line
  );
  ...
  const spriteColumn = <Box flexDirection="column" flexShrink={0} alignItems="center" width={colWidth} paddingTop={breathingPadding}>
...
export function CompanionFloatingBubble() {
  const $ = _c(8);
  const reaction = useAppState((s: AppState) => s.companionReaction) as string | undefined;
  let t0: { tick: number; forReaction: string | undefined };
  if ($[0] !== reaction) {
    t0 = {
      tick: 0,
